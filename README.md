...
- **듣지 않고 무음으로 녹음할 수 있습니다.** 오디오 MIDI 설정에서 **물리 출력 장치(스피커)
  ...
  참고)
  - **전사 정확도에는 영향 없습니다.** 다중 출력 기기는 장치별로 완전히 독립된 오디오
    스트림을 보내므로, 물리 스피커 슬라이더를 내려도 BlackHole로 들어가는 신호는 감쇠
    없이 그대로입니다. (반대로 `BlackHole 2ch` 슬라이더까지 같이 내리면 실제 녹음 신호가
    작아져 정확도에 영향을 줄 수 있으니 그 슬라이더는 건드리지 마세요.) 확인하려면:
