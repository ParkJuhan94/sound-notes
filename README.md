...
마이크 권한(TCC)이 백그라운드 실행에 대해 조용히 막히면, ffmpeg 프로세스는 죽지 않은 채
오디오 장치를 열지도 못하고 그대로 멈춰있을 수 있습니다 — 프로세스가 살아있으니
`note-status`도 계속 "녹음 중"으로 보이지만 실제로는 아무것도 기록되고 있지 않습니다
(2026-08-13 실제 발생, 16분 넘게 이 상태였음). 지금은 `note-start`가 시작 직후 WAV 파일이
실제로 생겼는지 확인하고, `note-status`도 파일이 15초 넘게 갱신되지 않으면 경고를 띄우도록
고쳐서 훨씬 빨리 잡아냅니다. 그래도 이 메시지를 보면: 시스템 설정 → 개인정보 보호 및 보안 →
마이크에서 터미널 앱 권한을 껐다 켜보고, 그래도 안 되면 재부팅 후 다시 시도하세요.
